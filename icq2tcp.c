...
void
sock_error_exit (sock_error, msg)
     bool sock_error;
     char * msg;
{
    if (sock_error) {
...
{    
    ...
    /* calculate checkcode */
    dword cc, cc2, code1, code2, data, pos;
    dword r1, r2, a[6];
    ...
    /* scamble packets */
    code1 = len * 0x68656c6cL + checkcode;
    ...
    for (pos = 0xa; pos < (len+3); pos+=4) {
        code2 = code1 + v5_table[pos & 0xff];
        data = chars_2_dw(buf+pos) ^ code2;
...
For example, %s icq.mirabilis.com 4000 4001\n\n", argv[0], argv[0]);
    ...
    large_sock = tcp_new_sock > udp_sock ? tcp_new_sock : udp_sock;
